...
def get_proper_observe_point_with_pathfinder(target_point_habitat, pathfinder, height):
    '''
    基于 Habitat 的导航器 pathfinder, 在目标点附近(半径1m内)找到一个可行走的, 且高度与height相差0.1m的点
    '''
    ...
    while True:
        ...
        try:
            # 调用 Habitat 的 API, 从目标点附近半径 1 米内, 随机采样一个可行走点
...
def get_random_observe_point(point, unoccupied_map, min_dist=15, max_dist=30):
    ...
    [min_dist, max_dist]: 限制观测点和目标点的距离范围
    '''
...
def get_proper_observe_point(point, unoccupied_map, cur_point, dist=10):
    '''
    在给定点 (point) 周围找到一个合适的观察点(observation point), 要求该点位于空闲区域, 不靠墙, 并且距离大约为 dist
    '''
...
def get_warping_gap(angles, tolerance_degree=30, max_try=1000):
    ...
    if angles[-1] - angles[0] > 1.95 * np.pi: 
        ...
        # 1. 随机选一个角度 ang，范围 [-π, π]
        # 2. 检查在 ang ± (tolerance/2) 的窗口内，是否有角度点
        # 3. 如果找到这样的 ang，就返回它
...
def adjust_navigation_point(
    ...
    '''
    对一个点 pos 进行调整, 使它远离障碍物(大于max_dist), 但又不会离当前位置太远(小于max_adjust_distance)
    pos是初始的 frontier 中心的位置(距离中心角度近, 且距离当前机器人位置近)
    '''
    ...
    # 如果这个位置被占据, 则找到距离这个位置最近的没有被unoccupied的位置
    if occupied[pos[0], pos[1]] == 1:
        # if the current position is occupied, we need to first find a nearby unoccupied position
        pos = get_nearest_true_point(pos, np.logical_not(occupied)) # 找到离输入点最近的 True 点

    # 找到距离调整后的中心位置最近的 已经被占据的点, 获取这个点相对这个位置的单位方向向量
    ...
    while count < max_try:
        ...
        # 返回None表示出界了, 如果还没找到更合适的位置, 返回原来的位置
        ...
        # 如果距离障碍物距离大于目标距离, 则搜索完毕
        if new_dist >= max_dist:
            break
        # 如果调整后位置距离初始位置太远, 超过了调整的范围, 则返回当前的搜索位置
        if np.linalg.norm(new_pos_int - pos) >= max_adjust_distance:
            break
        # 上面两种情况未发生, 即在搜索途中, 会每步都更新与障碍物的最短距离.
        if new_dist >= min_dist:
            min_dist = new_dist
        else:
            # 如果某一步, 最短障碍距离 减少了, 则回退到上一步的位置, 然后停止搜索
            new_pos = new_pos + direction * step_size
            new_pos_int = np.round(new_pos).astype(int)
            break

        # 更新搜索方向为 新位置和距离新位置最近的障碍物点 两个位置的方向向量的反方向
        # update direction
...
def get_collision_distance(occupied_map, pos, direction, max_step=50):
    # 给定起点和方向, 在占据地图里沿着该方向行进, 直到碰到障碍物或走到 max_step 为止, 返回行进的距离
    ...
    while count < max_step:
        ...
        if (
            0 <= curr_pos[0] < scene_bound[0]      # 没出界
            and 0 <= curr_pos[1] < scene_bound[1]  # 没出另一个边界
            and occupied_map[int(curr_pos[0]), int(curr_pos[1])]  # 被占据, 表示有障碍物
        ):
            break
